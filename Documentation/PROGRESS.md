...
- **Result/fee guardrail fixes (migration `20260624000001_report_signatures_fee_lock.sql`):**
  - **Fee guardrail bypass fixed** — `getPrintableReportCard` now enforces the fee lock server-side (parents with dues can no longer reach `/report-card/[id]/print` directly). Made the lock **configurable** via `schools.lock_results_on_fee` (default false; toggle in Settings → General).
  - **`getChildFeeStatus` balance bug fixed** — was summing only the 5 most recent payments; now uses a shared `computeStudentFeeBalance` helper (`src/lib/fees/balance.ts`) summing all payments.
  - **Signature embedding** — `schools.principal_signature_url` + `teachers.signature_url` print on report cards (class-teacher resolved via `teacher_section_assignments`); fall back to a blank signature line.
  - Server-side scholastic-marks validation + pre-save client validation; PDF accuracy fixes (dynamic academic session, grade legend from `grading_rules`, co-scholastic note); tie-aware rank ordering; bulk **"Print all report cards"** route `/report-card/class/[classId]/print`; father/mother names on the card. Shared `ReportCardDocument` component.
- **Milestone 1 — report-card & locale config (migration `20260624000002_school_customization.sql`):** `schools.report_card_title`, `pass_percentage`, `result_statuses[]`, `co_scholastic_grades[]`, `currency_symbol`, `locale`, `date_format`. Wired into the printed card (title badge, locale dates, auto Pass/Fail vs pass %) and the marks editor (result-status + co-scholastic dropdowns). All editable in Settings → General.
- **Phase 2 — report-card structure config (migration `20260624000003_report_card_config.sql`):** `subjects.display_order` (orders subjects on cards + marks entry; editable in Settings → Subjects); `schools.report_grand_total_rule` (average vs sum of terms); `schools.scholastic_component_labels` (rename Periodic Test / Notebook / Sub-Enrichment / Half-Yearly / Yearly); new `co_scholastic_areas` table (add/remove custom areas, falls back to the default trio). New **Settings → Report Card** tab.
- **Phase 3 — access control (migration `20260624000004_access_control.sql`):**
  - **Module visibility** — `schools.disabled_features[]` lets an admin switch any module off school-wide; wired into `usePlan().hasFeature` (auto-hides nav) and `requireFeature` (server redirect).
  - **Granular role permissions** — `role_permissions` table + permission catalog (`src/lib/permissions.ts`), server guard `requirePermission()` (`src/lib/auth/permissions.ts`), client `usePermissions().can()`, effective permissions hydrated into the auth store at sign-in. New **Settings → Access Control** tab = module toggles + a role × capability matrix (School Admin locked to full access).
  - **Enforcement** wired into report cards (enter marks, publish), fees (collect, configure), inventory (×5), teachers (×3), students (create/edit/delete), transport (×7), timetable (×5), attendance (mark). Defaults mirror each action's prior role gate (cashier defaults gained inventory access to avoid a regression).
